...
function attach(app){
  ...
  app.use((req,res,next)=>{res.setHeader('Content-Security-Policy',`default-src 'self'; script-src 'self' https://www.paypal.com https://www.sandbox.paypal.com; style-src 'self' 'unsafe-inline'; img-src 'self' data: https://miro.medium.com https://www.paypal.com https://www.sandbox.paypal.com; connect-src 'self' ws: wss: https://www.paypal.com https://www.sandbox.paypal.com https://api-m.paypal.com https://api-m.sandbox.paypal.com; frame-src 'self' https://www.paypal.com https://www.sandbox.paypal.com; media-src 'self' blob:; font-src 'self' data:; object-src 'none'; frame-ancestors 'none'; form-action 'self' https://www.paypal.com https://www.sandbox.paypal.com`);next()});
  const dataDir=path.join(__dirname,'data');fs.mkdirSync(dataDir,{recursive:true});const db=new Database(path.join(dataDir,'app.db'));
  ...
  app.post('/api/paypal/orders',auth,(req,res)=>{
    try{
      const id=Number(req.body?.consultationId);const c=db.prepare('SELECT c.*,s.name service_name,s.price_cents FROM consultations c JOIN users u ON u.id=c.user_id JOIN services s ON s.id=c.service_id WHERE c.id=? AND c.user_id=?').get(id,req.session.userId);
