(()=>{
  ...
  async function token(){
    if(!csrf){const r=await fetch('/api/csrf',{credentials:'same-origin'});if(!r.ok)throw Error('Impossible de sécuriser la messagerie.');csrf=(await r.json()).csrfToken}
    return csrf;
  }
  async function request(url,opt={}){
    ...
    const r=await fetch(url,o);let d={};try{d=await r.json()}catch{}if(!r.ok)throw Error(d.error||'Impossible de contacter la messagerie.');return d;
  }
  async function ensureIdentity(){if(window.__ladcUserId!=null)return;const d=await request('/api/me');window.__ladcUserId=d.user?.id??null}
  ...
  async function openChat(id){
    ...
    document.getElementById('chatClose').onclick=close;modal.addEventListener('click',e=>{if(e.target===modal)close()});
    function updateParticipant(c){
      const title=document.getElementById('chatTitle');
      const subtitle=document.getElementById('chatSubtitle');
      if(!title||!c)return;
      const first=String(c.clientFirstName||c.participant?.firstName||'').trim();
      const full=String(c.clientName||c.participant?.name||'').trim();
      const isOwner=window.__ladcUserRole==='owner';
      const name=isOwner?(first||full||'la personne'):'L’Art de la Connexion';
    ...
    async function load(){
      ...
      try{const d=await request(`/api/chat/${encodeURIComponent(active)}/messages?after=${lastId}`);if(!window.__ladcUserId)await ensureIdentity();updateParticipant(d.consultation);if(d.messages?.length)render(d.messages);else if(lastId===0)render([])}
      catch(e){if(!body.querySelector('.chat-error'))body.innerHTML=`<div class="chat-error">${esc(e.message)}</div>`}finally{loading=false}
    }
    try{const me=await request('/api/me');window.__ladcUserId=me.user?.id??null;window.__ladcUserRole=me.user?.role??null;await load()}catch(e){body.innerHTML=`<div class="chat-error">${esc(e.message)}</div>`}
    timer=setInterval(load,2500);
    document.getElementById('chatForm').onsubmit=async e=>{e.preventDefault();const text=input.value.trim();if(!text)return;const b=e.submitter;b.disabled=true;try{const d=await request(`/api/chat/${active}/messages`,{method:'POST',body:{body:text}});input.value='';render([d.message])}catch(e){window.alert(e.message)}finally{b.disabled=false;input.focus()}};
    document.getElementById('chatPick').onclick=()=>file.click();file.onchange=()=>file.files[0]&&upload(file.files[0]);
    ['dragenter','dragover'].forEach(x=>drop.addEventListener(x,e=>{e.preventDefault();drop.classList.add('dragging')}));['dragleave','drop'].forEach(x=>drop.addEventListener(x,e=>{e.preventDefault();drop.classList.remove('dragging')}));
    ...
    async function upload(f){
      ...
      try{const d=await request(`/api/chat/${active}/upload`,{method:'POST',body:fd});render([d.message])}catch(e){window.alert(e.message)}finally{btn.disabled=false;btn.textContent='Choisir une image';file.value=''}
    }
    input.focus();
  }
  document.addEventListener('click',e=>{const b=e.target.closest('[data-chat]');if(b){e.preventDefault();openChat(b.dataset.chat)}});
  const observer=new MutationObserver(injectButtons);observer.observe(document.getElementById('app')||document.body,{childList:true,subtree:true});setTimeout(injectButtons,500);window.LADCChat={open:openChat};
})();
