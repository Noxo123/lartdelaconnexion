(()=>{
  ...
  function mobileControls(win){
    const head=win?.querySelector('.chat-head');
    if(!head||head.querySelector('#clientNotesToggle'))return;
    head.querySelector('.chat-head>div')?.insertAdjacentHTML('afterend','<button type="button" class="client-notes-toggle" id="clientNotesToggle" aria-expanded="false" aria-controls="clientNotesPanel"><span>✦</span><b>Notes client</b></button>');
  }
  function bindDrawer(win){
    const panel=document.getElementById('clientNotesPanel'),toggle=document.getElementById('clientNotesToggle'),close=document.getElementById('clientNotesClose');
    if(!panel||!toggle)return;
    const setOpen=open=>{panel.classList.toggle('is-open',open);toggle.classList.toggle('is-open',open);toggle.setAttribute('aria-expanded',String(open));win.classList.toggle('notes-drawer-open',open);document.body.classList.toggle('notes-drawer-open',open)};
    toggle.onclick=()=>setOpen(!panel.classList.contains('is-open'));if(close)close.onclick=()=>setOpen(false);
  ...
  async function load(){
    ...
    mobileControls(win);bindDrawer(win);
